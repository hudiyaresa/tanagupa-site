...
export default function PermitForm() {
  ...
  const { step, personalInfo, permitDetails, documents } = useSelector(
    (state: RootState) => state.permit
  );
  ...
  // Step 3: Insert Link to Documents
  const handlePermitLink = (e: React.FormEvent) => {
    e.preventDefault();
    const link = (e.target as any).document_link.value;
    dispatch(
      setDocuments({
        fileName: "Linked Document",
        fileUrl: link,
      })
    );
    dispatch(setStep(4));
  ...
  return (
    <div className="max-w-xl mx-auto py-8 px-4">
      ...
      {step === 2 && (
        <form onSubmit={handlePermitDetails} className="space-y-4">
          <input
            ...
            defaultValue={permitDetails.park}
            required
          />
          <input
            name="date"
            type="date"
            className="w-full border rounded px-3 py-2"
            defaultValue={permitDetails.date}
            required
          />
          <input
            name="duration"
            ...
            defaultValue={permitDetails.duration}
            required
          />
          <button type="submit" className="bg-blue-600 text-white px-6 py-2 rounded">
            Next
      ...
      {step === 3 && (
        <form onSubmit={handlePermitLink} className="space-y-4">
          <input
            name="document_link"
            className="w-full border rounded px-3 py-2"
            placeholder="Document URL (e.g. Google Drive link)"
            defaultValue={documents.fileUrl}
            required
          />
          <button type="submit" className="bg-blue-600 text-white px-6 py-2 rounded">
            Next
